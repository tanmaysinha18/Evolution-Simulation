...
class World():
  ...
  def initialize_creatures(self, number_of_prey,number_of_predator):
    for i in range(0, number_of_prey):
      new_prey = Prey([random.uniform(0,self.x_range),random.uniform(0,self.y_range)],weights_food=np.random.uniform(-1,1,(4,10)))
      pos = new_prey.getPos()
      self.num_prey += 1
    ...
    for i in range(0, number_of_predator):
      self.predators=np.hstack((self.predators,Predator([random.uniform(0,self.x_range),random.uniform(0,self.y_range)],weights_food=np.random.uniform(-1,1,(2,6)))))
  ...
  def reset_creatures(self):
        ...
        for creature in self.prey_blocks[i][j]:
          if creature.fertility > 0:
            p = Prey(creature.getPos(),creature.speed+np.random.randint(-10,10),
                sense = creature.sense + np.random.uniform(-2,2), 
                weights_food=creature.nn_food.weights + np.random.normal(0,1,creature.nn_food.weights.shape))
    ...
    for creature in self.predators:
      if creature.fertility > 0:
        new_predators.append(Predator(creature.getPos(),
          creature.speed+np.random.randint(-10,10),
          sense = creature.sense+ np.random.uniform(-2,2),
          weights_food=creature.nn_food.weights + np.random.normal(0,1,creature.nn_food.weights.shape)))
